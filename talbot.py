...
def get_perspective(wavelength=config.wavelength, period=config.period,
                    phase_shift=config.phase_shift, show_z_talbot=config.show_z_talbot):
    x = np.linspace(config.start_x_axis, -config.start_x_axis, config.plot_resolution_x)
    z = np.linspace(config.start_z_axis, end_z_axis, config.plot_resolution_y)

    u0 = Scalar_source_X(x, wavelength)
    u0.plane_wave(A=1)

    t = Scalar_mask_X(x, wavelength)
    t.ronchi_grating(x0=phase_shift, period=period, fill_factor=0.5)
    ...
    plt.savefig(config.image_folder + config.picture_name)


def get_front(z_indent):
    y = np.linspace(-config.start_x_axis, config.start_x_axis, config.plot_resolution_y)
    x = np.linspace(-config.start_x_axis, config.start_x_axis, config.plot_resolution_x)
    ...
    t1.ronchi_grating(x0=phase_shift - 10, period=period, fill_factor=0.5, angle=90 * degrees)
    ...
    plt.savefig(config.image_folder + config.picture_front_name)
...
end_z_axis = int((z_talbot * 1.6 + 99) / 100) * 100  # pushing z axis to have enough space to picture z_talbot
